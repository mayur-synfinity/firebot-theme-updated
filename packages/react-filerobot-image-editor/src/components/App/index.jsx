...
const App = () => {
  ...
  const renderContent = () => (
    <>
      ...
      {originalImage && feedback.duration !== 0 && (
        <StyledMainContent isDarkMode={isDarkMode} className="FIE_main-container">
          {!showCanvasOnly && !showTabsDrawer && (
            <StyledTabs isDarkMode={isDarkMode} className="FIE_tabs">
  ...
  return (
    <StyledAppWrapper
      ...
      $size={rootSize}
    >
      {isLoadingGlobally && <Spinner isDarkMode={isDarkMode} theme={theme} />}
      {renderContent()}
      <FeedbackPopup />
    </StyledAppWrapper>
  );
